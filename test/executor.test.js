...
test('配置校验并按 host、port 生成固定回程 URL', () => {
  ...
  assert.deepEqual(
    readConfig(baseEnv),
    { port: 18033, ingressHost: '127.0.0.1', ingressPort: 29876, ingressToken: 'test', ingressTimeoutMs: 5000, ingressUrl: 'http://127.0.0.1:29876/agent/send' }
  );
...
test('正式剧本完整顺序、数量与完整周期均正确', async () => {
  ...
  assert.equal(waits.filter((ms) => ms === 40000).length, 15);
  assert.equal(waits.filter((ms) => ms === 720000).length, 12);
  assert.equal(waits.reduce((sum, ms) => sum + ms, 0), 9240000);
});

test('Mock 回程逐条收到正式能效文案、固定映射与协议字段', async () => {
  const requests = [];
  ...
  try {
    ...
    assert.deepEqual(requests.map((request) => request.body), expectedBodies.map((body) => ({
      agent: 'xslatdzp.EnrOpsAgent', to: 'xslatdzp.demo001@rscom-chat.rsagent.net', body, groupchat: false
    })));
